...
describe("Vault", function () {
  ...
  async function deployFixture() {
    ...
    //Token deploy
    const Token = await ethers.getContractFactory("Simple404", owner);
  ...
  it("Should deposit into the Vault", async () => {
    ...
    expect(await token.balanceOf(vault.getAddress())).equal(parseEther(10));
  });
  ...
  it("Should withdraw", async () => {
    ...
    expect(await token.balanceOf(vault.getAddress())).equal(parseEther(7));
    expect(await token.balanceOf(alice.address)).equal(parseEther(93));
  });
  ///////Unhappy Path/////////
